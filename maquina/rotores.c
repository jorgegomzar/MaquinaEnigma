...
void genWalzenlage(int nrotor, unsigned short walzen[])
{
    ...
    if(nrotor==3)/*GENERA 3 NUMEROS ALEATORIOS Y DISTINTOS ENTRE SI ENTRE 1 Y 8 INCLUSIVE Y SE ASIGNAN A WALZEN*/
    {
        for (i=0; i<nrotor; i++)
        {
            num=1+rand()%8;
    ...
    if(nrotor==4)/*GENERA 3 NUMEROS ALEATORIOS Y DISTINTOS ENTRE SI ENTRE 1 Y 8 INCLUSIVE Y UN CUARTO ROTOR ALEATORIO ENTRE BETA Y GAMMASE ASIGNAN A WALZEN*/
    {
...
void fprintWalzenlage(int nrotor, unsigned short walzen[], FILE *ptr, FILE *pCfg)
{
    ...
    if(nrotor==4)/*EN EL CASO DE HABER 4 ROTORES EL PROCEDIMIENTO SERIA PARECIDO PERO AUMENTANDO EL TAMAÑO DE LA CADENA*/
    {
        for(i=0; i<21; i++)
        {
            output[i]=' ';
        ...
        if(walzen[3]==9)/*DEPENDIENDO DEL VALOR DEL 4 ROTOR ESCRIBO BETA O GAMMA*/
        {
            output[0]='B';
            output[1]='e';
            output[2]='t';
...
void leeCfgRotor(struct sRotor * pRotor)
{
    ...
    ptx=fopen("./BASEDATOS/cableadoRotores.cfg", "rt");/**ABRO EL ARCHIVO DE LOS CABLEADOS Y SI HAY UN ERROR AVISA AL USUARIO*/
    if(ptx==NULL)
    {
        system("cls");
        system("pause");
    ...
    if(pRotor->num<=5)/*SI EL NUMERO DEL ROTOR ES MENOR O IGUAL QUE 5 COLOCO EL PUNTERO AL PRINCIPIO DE SU LINEA CORRESPONDIENTE*/
    {
        longitud=30*(pRotor->num-1);
        fseek(ptx,longitud,0);
    }
    else/*SI ES MAYOR, COJO EL PUNTERO AL PRINCIPIO DE SU LINEA CORRESPONDIENTE*/
    {
    ...
    fgets(pRotor->cableado, 27, ptx);/*FUNCIONA PARA LA SEGUNDA Y LA TERCERA CADENA PERO TENGO PROBLEMAS CON LA PRIMERA*/
    pRotor->cableado[26]=0;
    fseek(ptx,sizeof(char),SEEK_CUR);

    if(pRotor->num<9)/**SI EL ROTOR NO ES EL 9 NI EL 10 LEE LA MUESCA*/
    {
        if(pRotor->num<=5)
        {
...
void 	configAnillos (struct sRotor * pRotor, int pos)/**NO ESTOY SEGURO DE SI INICIALIZAR ANILLO Y REFERENCIA AL ABECEDARIO*/
{
    ...
    for(i=0; i<26; i++)/*SUMO A TODAS LAS LETRAS DE CADA ROTOR POS-1 Y SI SOBREPASAN LA Z HAGO QUE DEN LA VUELTA AL ABECEDARIO*/
    {
...
void 	dibujaRotor (struct sRotor rotor, struct sPunto coordRotor)
{
    ...
    char output[5];
    gotoxy(x,y);
    printf("%c", rotor.anillo[0]);/*ESCRIBO EL PRIMER CARACTER DE RINGSTELLUNG*/
    x-=2;
    y+=3;
    setColorTexto(206);
    for(i=0; i<26; i++)/*IMPRIME EL ANILLO EN UN COLOR DISTINTO Y MARCA LAS MUESCAS DE OTRO COLOR*/
    {
        gotoxy(x,y);
    ...
    y=coordRotor.y+3;
    for(i=0; i<26; i++)/*IMPRIME EL CABLEADO*/
    {
        gotoxy(x+1,y);
    ...
    y-=26;
    x++;
    for(i=0; i<26; i++)/*IMPRIME LA REFERENCIA*/
    {
        gotoxy(x+1,y);
    ...
    {
    case 1:
        /*EL PRIMER NUMERO SE GUARDA EN LA PRIMERA POSICION, DESPUÉS TODOS LOS DEMÁS SE ASIGNAN EN LAS SIGUIENTES*/
        output[i]='I';
        break;
    case 2:
        output[i]='I';
